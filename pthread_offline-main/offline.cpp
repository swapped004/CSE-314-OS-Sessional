...
sem_t left_right_mutex;
sem_t right_left_block;
sem_t left_right_block;

sem_t left_right_pending;
sem_t right_left_mutex;
...
void* left_right_path(void *arg)
{
	...
	sem_wait(&left_right_mutex);
	left_right_passengers++;
	if(left_right_passengers == 1)
	{
		sem_wait(&right_left_block);
	}
	sem_post(&left_right_mutex);

	sem_wait(&left_right_block);
	sem_post(&left_right_block);
...
void* right_left_path(void *arg)
{
	...
	sem_wait(&left_right_pending);

	sem_wait(&right_left_block);

	sem_wait(&right_left_mutex);
	right_left_passengers++;
	if(right_left_passengers == 1)
	{
		sem_wait(&left_right_block);
	}
	sem_post(&right_left_mutex);

	sem_post(&right_left_block);

	sem_post(&left_right_pending);
	...
	pthread_t left_right_path_thread;
	pthread_create(&left_right_path_thread,NULL,left_right_path,(void*)new struct info(temp));
	pthread_join(left_right_path_thread,NULL);
...
void* boarding_func(void *arg)
{
	...
	else //go back through VIP line
	{
		...
		pthread_t right_left_path_thread;
		pthread_create(&right_left_path_thread,NULL,right_left_path,(void*)new struct info(temp));
		pthread_join(right_left_path_thread,NULL);
...
{	
	...
	//VIP gate semaphores
	sem_init(&left_right_mutex,0,1);
	sem_init(&right_left_mutex,0,1);
	sem_init(&left_right_block,0,1);
	sem_init(&right_left_block,0,1);
	sem_init(&left_right_pending,0,1);
